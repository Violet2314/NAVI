...
if __name__ == "__main__":
    # 注意：PyInstaller 打包后不能用 "main:app" 字符串（会触发 import main 失败）。
    # 直接传 app 对象，reload/workers 在 frozen 下也必须关闭。
    import sys as _sys
    import os as _os
    ...
    def _crash_log_path() -> Path:
        """frozen 下把崩溃日志落到 %APPDATA%\\navi\\backend-crash.log（Tauri 吞 stderr 时也能看到真实错误）"""
        appdata = _os.environ.get("APPDATA") or _os.environ.get("HOME") or "."
        log_dir = Path(appdata) / "navi"
        try:
            log_dir.mkdir(parents=True, exist_ok=True)
        except Exception:
            log_dir = Path(".")
        return log_dir / "backend-crash.log"

    try:
        # 打包后把标准错误也 tee 到崩溃日志，帮助定位 sidecar 拉起失败的根因
        if _frozen:
            _log_path = _crash_log_path()
            _log_fp = open(_log_path, "a", encoding="utf-8", buffering=1)
            _log_fp.write(f"\n========== {_os.path.basename(_sys.executable)} starting ==========\n")
            print(f"[navi-backend] crash log: {_log_path}", flush=True)
        else:
            _log_fp = None
        ...
        _msg = f"[navi-backend] fatal: {type(_e).__name__}: {_e}\n{_tb.format_exc()}"
        try:
        ...
        try:
            ...
            pass
        # 退出码 != 0 让 Tauri 那边能在 CommandEvent::Terminated 里拿到非零 code
        _sys.exit(1)
