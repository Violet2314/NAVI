...
# chat() 通用文本对话
LLM_CHAT_MAX_TOKENS = 20480
LLM_CHAT_TEMPERATURE = 0.7

# vision_chat() 单图理解
LLM_VISION_MAX_TOKENS = 3000

# vision_batch() 多图批量
LLM_VISION_BATCH_MAX_TOKENS = 6000
...
CLASSIFIER_BASE_MAX_TOKENS = 20480

# 活动分类：每条活动的 token 预算乘数
CLASSIFIER_PER_ACTIVITY_TOKENS = 40

# 截图理解（vision_chat 调用）
CLASSIFIER_SCREENSHOT_MAX_TOKENS = 5000

# gap_detector — 日报追问判断（单次调用，需为思维链模型留余量）
GAP_DETECTOR_MAX_TOKENS = 5120
...
# 事实矛盾检测
MEMORY_CONTRADICTION_MAX_TOKENS = 2560
MEMORY_CONTRADICTION_TEMPERATURE = 0.0

# 对话→事实提取
MEMORY_EXTRACTION_MAX_TOKENS = 40960
...
REPORT_MAX_TOKENS = 131072

# 截图批量描述：基础 token（实际 = max(600, 截图数 * (MAX_DESC_LEN + 20))）
REPORT_VISION_DESC_BASE_TOKENS = 6000
...
# 素材分析（文本类）
SOUL_ANALYZER_TEXT_MAX_TOKENS = 40960
SOUL_ANALYZER_TEXT_TEMPERATURE = 0.3

# 素材分析（图片类）
SOUL_ANALYZER_IMAGE_MAX_TOKENS = 40960

# 首次合成 SOUL.md
SOUL_SYNTHESIZER_MAX_TOKENS = 81920
SOUL_SYNTHESIZER_TEMPERATURE = 0.5

# 增量修改 SOUL.md（用户反馈后微调）
SOUL_REFINE_MAX_TOKENS = 81920
...
AGENT_DEFAULT_MAX_TOKENS = 40960
...
PROACTIVE_GENERATOR_MAX_TOKENS = 1500
