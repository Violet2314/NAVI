...
def _is_sensitive_path(path: Path) -> bool:
    ...
    if name in _SENSITIVE_PATH_PARTS:
        return True
    # 检测以 .pem / .key / .p12 结尾的证书/密钥文件
    if name.endswith((".pem", ".key", ".p12", ".pfx", ".cer")):
        return True
    return False
    ...
    async def execute(self, path: str, offset: int = 1, limit: int | None = None, **kwargs: Any) -> str:
        try:
            ...
            if _is_sensitive_path(fp):
                return (
                    f"⛔ 访问被拒绝：{path} 包含敏感凭据路径（私钥/云凭据/证书），"
                    "出于安全原因 Navi 不会读取此文件。"
                )
