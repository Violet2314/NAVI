...
#
# 核心原则：Soul 是日报的"操作系统"，模板只是"文件格式"。不要用模板压制 Soul。但是需要注意的是。
_REPORT_TASK_RULES = """---
...
【格式约束 — 仅此三条，不要自己加规则】
1. 输出完整 Markdown 文档 不需要标题，因为我是obsidan的md笔记
2. 可以包含你的标志性动作描写（用括号），可以包含你的语气词和口癖——做你自己
3. 不要使用 [SPLIT] 分段标记（那是即时聊天用的，日报是完整文档）
4. 日报中不能出现任何emoji，并且日报中不能出现动作，比如（耳朵微微抖动，认真地翻阅记录）,并且日报还是要注意一定的格式化分块的就是需要多几个 ## 标题来分块
